...
import { Scenario, sellscenarios, buyscenarios, signTxnWithTestAccount } from "./scenarios";
...
const SConnectButton = styled(Button as any)`
  ...
  background-image: linear-gradient(90deg,#d502f6,#fab21c);
  height: 44px;
  width: 100%;
  margin: 12px 0;
`;
...
const asset_hash = params.get('asset_hash');
const collection_name = params.get('collection_name');
const asset_ipfs = 'https://ipfs.algonft.tools/ipfs/' + asset_hash


console.log(params.get('trade_id'));
...
class App extends React.Component<unknown, IAppState> {
  ...
  public async submitSignedTransaction() {
    const { signedTxns, chain, address } = this.state;
    if (signedTxns == null) {
    ...
    signedTxns.forEach(async (signedTxn, index) => {
      try {
        ...
        if (trade_id === 'sell') {
          const redirect_url = 'https://spotlight-explorer.herokuapp.com/gallery?seller=' + address + '&asset_id=' + asset_id + '&asset_price=' + asset_price + '&collection_name=' + collection_name + '&asset_hash=' + asset_hash;
  ...
  public render = () => {
    ...
    //   const mycolors = {
    //     $dark_color: #171316;
    //   $primary_color: #171316;
    //   $secondary_color: #F7A829;
    // $white_color: #ffffff;
    //   }
    const myStyle = {
      backgroundImage:
        "url(/static/images/ellipse-gradient.png)",
      height: '100vh',
                ...
                <SBalances>
                  ...
                  {trade_id === 'sell' ? (
                    <div>
                      ...
                      <p>Asset Price: {asset_price}</p>
                    </div>
                  ) : (
                    <div>
                      <h5>To Buy an NFT, you need to opt in first</h5>
                      ...
                      />
                      <p>Asset ID: {asset_id}</p>
                      <p>Asset Price: {asset_price}</p>                    </div>
                  )}
                  <Column center>

                    <STestButtonContainer>
                      {trade_id === 'sell' ? (
                        sellscenarios.map(({ name, scenario }) => (
                        ...
                        buyscenarios.map(({ name, scenario }) => (
                          <STestButton left key={name} onClick={() => this.signTxnScenario(scenario)}>
                            {name}
                          </STestButton>
                        ))
            ...
            {pendingRequest ? (
              ...
              <SModalContainer>
                ...
                {pendingSubmissions.map((submissionInfo, index) => {
                  const key = `${index}:${typeof submissionInfo === "number" ? submissionInfo : "err"
                    }`;
                  ...
                  if (submissionInfo === 0) {
                    ...
                    content = "Rejected by network. See console for more information." + submissionInfo;
                  }
